<?php
  ...
  $con = mysqli_connect($servername, $username, $password, $db);
    ...
    $ogrenciBilgi = ("select * from ogrenciBilgileri ");
    $ogrenciBilgiSQL = mysqli_query ($con, $ogrenciBilgi);
    $ogrenciBilgiDondur = mysqli_fetch_array($ogrenciBilgiSQL);
    $isimSoyisim = $ogrenciBilgiDondur["isimSoyisim"];
    $Fakulte = $ogrenciBilgiDondur["Fakultesi"];
    $Sinifi = $ogrenciBilgiDondur["Sinifi"];
    $kayitTarihi = $ogrenciBilgiDondur["kayitTarihi"];
    $stajDurum = $ogrenciBilgiDondur["stajDurumu"];
    $fotograf = $ogrenciBilgiDondur["fotograf"];
...
<html lang="tr">
  ...
  <body>
    ...
    <div class="row-icerik">
      <h5 style="padding: 10px; font-weight: bold; margin-bottom: -5px;">Öğrenci Bilgileri</h5>
      
      <?php
      echo '
        <img src="'.$fotograf.'" class="resimOgrenci">
        <p style="padding: 25px;">
                <strong>İsim ve Soyisim:</strong> '.$isimSoyisim.'<br>
                <strong>Fakülte ve Sınıf:</strong> '.$Fakulte.' '.$Sinifi.'<br>
                <strong>Okul Kayıt Tarihi:</strong> '.$kayitTarihi.'<br>
                <strong>Staj Arama Durumu:</strong> '.$stajDurum.'<br>
        </p>
      ';
     ?>

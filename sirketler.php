<?php
  ...
  $con = mysqli_connect($servername, $username, $password, $db);
    ...
    $kontrolIlan = ("select * from stajSirketleri ");
...
<html lang="tr">
  ...
  <body>
    ...
    <div class="row-icerik">
      <h5 style="padding: 10px; font-weight: bold; margin-bottom: -5px;">Staj Şirketleri</h5>
      <table style="margin-top:30px; margin-left: 5px;">
      <tr>
            <td><strong>Şirket Adı</strong></td>
            <td><strong>Konumu</strong></td>
            <td><strong>Şirket Kategorisi</strong></td>
            <td><strong>Alım Sayısı</strong></td>
        </tr>
       <?php 
       ...
       if ($kontrolIlanSQL-> num_rows > 0) {
        while ($row = $kontrolIlanSQL-> fetch_assoc()) {
            ...
            $sirketKonum = $row["sirketKonum"];
            $sirketKategori = $row["sirketKategori"];
            $sirketAlim = $row["sirketAlim"];

            echo'
            <tr>
                <td>'.$sirketAdi.'</td>
                <td>'.$sirketKonum.'</td>
                <td>'.$sirketKategori.'</td>
                <td>'.$sirketAlim.'</td>
            </tr>
            ';
        }
